...
var myTran = {
  ...
  */
  goTo: function (name, reverse, toPage, fromPage) {
    //myTran.animationend(toPage);
    //myTran.animationend(fromPage);
    myTran.transition(name, reverse, toPage, fromPage);
  },
  //添加转屏动画css
  ...
  restore: function (name, page) {
    $(page).removeClass(name + " out");
  },
  // 加载页面
				...
				.on(
						...
						function () {
						  if ($(this).attr('class').indexOf(" in") > 0) {
						    ...
						    $(this)
										.addClass(
												"ui-mobile-viewport-transitioning ui-page hide");
						  }
						});
  }
}